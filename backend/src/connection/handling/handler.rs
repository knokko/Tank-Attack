    ...
    fn on_message(&mut self, msg: ws::Message) -> Result<(), ws::Error> {
        if !msg.is_binary() {
            ...
            let close_result = self
                .out
                .close_with_reason(CloseCode::Unsupported, "Only binary messages are allowed");
            match close_result {
                Ok(_) => {}
                Err(error) => {
                    println!(
                        "Closing connection with non-binary websocket failed: {}",
                        error
                    );
                }
            };
...

        {
            ...
            if next_task.id == task_id {
                ...
                if next_task.queue.len() < MAX_MANUAL_QUEUE_LENGTH {
                    ...
                    let close_result = self.out.close_with_reason(
                        CloseCode::Error,
                        "Too many of your messages are queued, server is probably overloaded",
                    );
                    match close_result {
                        Ok(_) => {}
                        Err(error) => println!(
                            "Failed to close connection with handler with too long queue: {}",
                            error
                        ),
                    };
                }
